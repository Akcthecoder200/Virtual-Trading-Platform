...
router.get("/profile", authenticateToken, async (req, res) => {
  try {
    const user = await User.findById(req.user._id).select(
      "-password -security.refreshTokens"
    );
  ...
  async (req, res) => {
    try {
      ...
      if (updateData.firstName)
        profileUpdates["profile.firstName"] = updateData.firstName;
      if (updateData.lastName)
        profileUpdates["profile.lastName"] = updateData.lastName;
      if (updateData.phone) profileUpdates["profile.phone"] = updateData.phone;
      if (updateData.dateOfBirth)
        profileUpdates["profile.dateOfBirth"] = new Date(
          updateData.dateOfBirth
        );
      if (updateData.country)
        profileUpdates["profile.country"] = updateData.country.toUpperCase();
      if (updateData.bio) profileUpdates["profile.bio"] = updateData.bio;

      // Handle address updates
      if (updateData.address) {
        if (updateData.address.street)
          profileUpdates["profile.address.street"] = updateData.address.street;
        if (updateData.address.city)
          profileUpdates["profile.address.city"] = updateData.address.city;
        if (updateData.address.state)
          profileUpdates["profile.address.state"] = updateData.address.state;
        if (updateData.address.zipCode)
          profileUpdates["profile.address.zipCode"] =
      ...
      if (updateData.preferences) {
        if (updateData.preferences.theme)
          profileUpdates["preferences.theme"] = updateData.preferences.theme;
        if (updateData.preferences.language)
          profileUpdates["preferences.language"] =
            updateData.preferences.language;
        if (updateData.preferences.currency)
          profileUpdates["preferences.currency"] =
            updateData.preferences.currency;
        if (updateData.preferences.timezone)
          profileUpdates["preferences.timezone"] =
        ...
        if (updateData.preferences.notifications) {
          Object.keys(updateData.preferences.notifications).forEach((key) => {
            profileUpdates[`preferences.notifications.${key}`] =
...
router.get("/analytics", authenticateToken, async (req, res) => {
  try {
    ...
    const accountAge = Math.floor(
      (new Date() - user.analytics.signupDate) / (1000 * 60 * 60 * 24)
    );
    const avgLoginsPerDay =
      accountAge > 0 ? (user.analytics.totalLogins / accountAge).toFixed(2) : 0;
