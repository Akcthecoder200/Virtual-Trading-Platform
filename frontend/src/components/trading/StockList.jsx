...
import { addToWatchlist, removeFromWatchlist } from '../../store/slices/tradingSlice';
...
const StockList = ({ onSelectStock }) => {
  ...
  const formatChange = (change, changePercent) => {
    ...
    return {
      value: `${isPositive ? '+' : ''}${formatPrice(change)} (${isPositive ? '+' : ''}${changePercent.toFixed(2)}%)`,
        ...
        <table className="w-full">
          <thead>
            <tr className="border-b border-gray-700">
              ...
              <th className="text-left pb-3 text-gray-400 font-medium">Company</th>
              ...
              <th className="text-right pb-3 text-gray-400 font-medium">Volume</th>
          ...
          <tbody>
            {stocks.map((stock) => {
              ...
              return (
                <tr key={stock.symbol} className="border-b border-gray-700/50 hover:bg-gray-700/30 transition-colors">
                  <td className="py-4">
                    <button
                    ...
                    >
                      {isInWatchlist ? <Star className="w-4 h-4 fill-current" /> : <Star className="w-4 h-4" />}
                    </button>
                  </td>
                  ...
                  <td className="py-4 text-right">
                    <div className="flex items-center justify-end">
                      <span className={`inline-flex items-center gap-1 px-2 py-1 rounded-lg text-sm ${change.color} ${change.bgColor}`}>
                        {change.icon}
                        {change.value}
                      </span>
                    </div>
