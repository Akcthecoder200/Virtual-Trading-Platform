...
#### useMarketData Hook
```javascript
const { data, quote, loading, error, refresh } = useMarketData(symbol, interval, range);
```
...
```javascript
const { quotes, loading, refresh } = useMultipleQuotes(['AAPL', 'GOOGL', 'TSLA']);
```
...
<StockSearch
  onStockSelect={(stock) => console.log('Selected:', stock)}
/>
```
